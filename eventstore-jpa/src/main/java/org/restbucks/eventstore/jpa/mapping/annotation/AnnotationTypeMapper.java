...
import org.restbucks.eventstore.jpa.configuration.EventstoreProperties;
...
import static org.restbucks.eventstore.jpa.exception.ConflictingTypeMappingException.conflictingTypeMapping;
...
public class AnnotationTypeMapper implements TypeMapper {
    ...
    private void requireOneClassMappedToOneType(Class<?> a, Class<?> b) {
        if (classesMappedToSameType(a, b)) {
            final var type = classTypeExtraction(a);
            throw conflictingTypeMapping(a, b, type);
        }
    }

    private boolean classesMappedToSameType(Class<?> a, Class<?> b) {
        return !a.equals(b);
    }
    ...
    private String classTypeExtraction(Class<?> aClass) {
        return classRegistry
                ...
                .orElseThrow(() -> missingTypeMappingForClass(aClass));
    }
}
